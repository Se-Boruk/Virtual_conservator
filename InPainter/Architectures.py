...
class Inpainter_V4:
    ...
    class ResBlock(nn.Module):
        def __init__(self, channels):
            super().__init__()
            self.conv1 = nn.Conv2d(channels, channels, 3, 1, 1)
            self.norm1 = Inpainter_V4.groupnorm(channels)
            self.act1 = nn.LeakyReLU(0.2, inplace=True)
            
            self.conv2 = nn.Conv2d(channels, channels, 3, 1, 1)
            self.norm2 = Inpainter_V4.groupnorm(channels)
            self.skip_gain = 1 / (2 ** 0.5)
    ...
    #============================================================
    class Processing_Block(nn.Module):
        def __init__(self, input_channels, channels, fusion_groups=8):
            super().__init__()
            half_c = channels // 2
    
            # Early conv block
            self.conv_1 = nn.Conv2d(input_channels, channels, 3, 1, 1)
            self.norm_1 = nn.GroupNorm(num_groups=max(1, channels // 8), num_channels=channels)
            self.act_1 = nn.LeakyReLU(0.2, inplace=True)
    
            # Heads (dilated) GroupNorm per channel acts as layernorm but no need for special permute and etc.
            self.head1 = nn.Sequential(
                nn.Conv2d(channels, half_c, 3, 1, 1, dilation=1, groups=half_c),
                nn.GroupNorm(num_groups=half_c, num_channels=half_c),
                nn.Conv2d(half_c, half_c, 1, 1, 0),
                nn.GroupNorm(num_groups=half_c, num_channels=half_c),
                nn.LeakyReLU(0.2, inplace=True)
            )
            ...
            # Fusion convs
            self.conv_61 = nn.Conv2d(half_c * 3, channels, kernel_size=1, stride=1, padding=0)
            self.norm_61 = nn.GroupNorm(num_groups=max(1, channels // 8), num_channels=channels)
            self.act_61 = nn.LeakyReLU(0.2, inplace=True)
    
            self.conv_71 = nn.Conv2d(channels, channels, kernel_size=3, stride=1, padding=1)
            self.norm_71 = nn.GroupNorm(num_groups=max(1, channels // 8), num_channels=channels)
            self.act_71 = nn.LeakyReLU(0.2, inplace=True)
    ...
    class Downsample_block(nn.Module):
        def __init__(self, in_channels, out_channels):
            ...
            self.conv_stride = nn.Conv2d(in_channels, in_channels, 3, stride=2, padding=1)
            self.norm1 = nn.GroupNorm(num_groups=max(1, in_channels // 8), num_channels=in_channels)
            self.act1 = nn.LeakyReLU(0.2, inplace=True)
    
            # Extra conv for sharpening / detail refinement
            self.conv_extra = nn.Conv2d(in_channels, in_channels, 3, 1, 1)
            self.norm_extra = nn.GroupNorm(num_groups=max(1, in_channels // 8), num_channels=in_channels)
            self.act_extra = nn.LeakyReLU(0.2, inplace=True)
        ...
        def forward(self, x):
            x = self.act1(self.norm1(self.conv_stride(x)))
            x = self.act_extra(self.norm_extra(self.conv_extra(x)))
            x = self.act2(self.norm2(self.conv_1x1(x)))
            return x
    ...
    class Encoder(nn.Module):
        def __init__(self, input_channels=3, base_filters=32, n_residual=4):
            ...
            self.enc_0 = Inpainter_V4.Processing_Block(f1, f1)
            self.down_0 = Inpainter_V4.Downsample_block(f1, f2)
            self.enc_1 = Inpainter_V4.Processing_Block(f2, f2)
            self.down_1 = Inpainter_V4.Downsample_block(f2, f3)
            self.enc_2 = Inpainter_V4.Processing_Block(f3, f3)
            self.down_2 = Inpainter_V4.Downsample_block(f3, f4)
            self.enc_3 = Inpainter_V4.Processing_Block(f4, f4)
            self.down_3 = Inpainter_V4.Downsample_block(f4, f5)

            self.resblocks = nn.Sequential(*[Inpainter_V4.ResBlock(f5) for _ in range(n_residual)])
    ...
    class Decoder(nn.Module):
        def __init__(self, output_channels=3, base_filters=32):
            ...
            # Upsample blocks
            self.up_3 = Inpainter_V4.Upsample_block(f5 + 8, f4)  # +8 for class_map
            self.dec_3 = Inpainter_V4.Processing_Block(f4, f4)  # only f4, skip added separately
            self.up_2 = Inpainter_V4.Upsample_block(f4, f3)
            self.dec_2 = Inpainter_V4.Processing_Block(f3, f3)
            self.up_1 = Inpainter_V4.Upsample_block(f3, f2)
            self.dec_1 = Inpainter_V4.Processing_Block(f2, f2)
            self.up_0 = Inpainter_V4.Upsample_block(f2, f1)
            self.dec_0 = Inpainter_V4.Processing_Block(f1, f1)
    
            # Class mapping
            self.class_mapping = Inpainter_V4.Class_MLP(
                in_dim=1, hidden_dim=int(base_filters*8),
                out_channels=8, out_h=8, out_w=8
            )
            ...
            self.skip_params = nn.Parameter(torch.full((4,), -2.0))
        ...
        def forward(self, x, skip_0, skip_1, skip_2, skip_3, class_vector):
            ...
            skip_w = torch.sigmoid(self.skip_params)
...
class Inpainter_V5:
    ...
    class ResBlock(nn.Module):
        def __init__(self, channels):
            super().__init__()
            self.conv1 = nn.Conv2d(channels, channels, 3, 1, 1)
            self.norm1 = Inpainter_V5.groupnorm(channels)
            self.act1 = nn.LeakyReLU(0.2, inplace=True)
            
            self.conv2 = nn.Conv2d(channels, channels, 3, 1, 1)
            self.norm2 = Inpainter_V5.groupnorm(channels)
            self.skip_gain = 1 / (2 ** 0.5)
    ...
    class Encoder(nn.Module):
        def __init__(self, input_channels=3, base_filters=32, n_residual=4):
            ...
            self.enc_0 = Inpainter_V5.Processing_Block(f1, f1)
            self.down_0 = Inpainter_V5.Downsample_block(f1, f2)
            self.enc_1 = Inpainter_V5.Processing_Block(f2, f2)
            self.down_1 = Inpainter_V5.Downsample_block(f2, f3)
            self.enc_2 = Inpainter_V5.Processing_Block(f3, f3)
            self.down_2 = Inpainter_V5.Downsample_block(f3, f4)


            self.resblocks = nn.Sequential(*[Inpainter_V5.ResBlock(f4) for _ in range(n_residual)])
    ...
    class Decoder(nn.Module):
        def __init__(self, output_channels=3, base_filters=32):
            ...
            self.up_2 = Inpainter_V5.Upsample_block(f4 + 8, f3) # +8 for class_map
            self.dec_2 = Inpainter_V5.Processing_Block(f3, f3)
            self.up_1 = Inpainter_V5.Upsample_block(f3, f2)
            self.dec_1 = Inpainter_V5.Processing_Block(f2, f2)
            self.up_0 = Inpainter_V5.Upsample_block(f2, f1)
            self.dec_0 = Inpainter_V5.Processing_Block(f1, f1)
    
            # Class mapping
            self.class_mapping = Inpainter_V5.Class_MLP(
                in_dim=1, hidden_dim=int(base_filters*8),
                out_channels=8, out_h=16, out_w=16
            )
    
            # Merge after class-map concat
            self.merge = nn.Sequential(
                nn.Conv2d(f4 + 8, f4 + 8, 1),
                nn.InstanceNorm2d(f4 + 8, affine=True),
                nn.LeakyReLU(0.2, inplace=True)
            )
            ...
            self.skip_params = nn.Parameter(torch.full((3,), -2.0))
